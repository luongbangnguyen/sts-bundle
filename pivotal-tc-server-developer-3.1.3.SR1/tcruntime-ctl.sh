...
getJAVA_VERSION () {
  ...
  if [ -n $JAVA_HOME ]; then
    ...
    JAVA_VERSION=`$JAVA_HOME/bin/java -cp "$CATALINA_HOME/../lib/tc-runtime-instance-3.1.3.SR1.jar" com.springsource.tcruntime.instance.internal.util.TcJavaVersionTester --getVersionAsInteger`
    testJAVA_VERSION
  else
